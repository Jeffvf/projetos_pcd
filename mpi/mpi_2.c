...
/* Struct para cálculo de intervalo de tempo */
typedef struct {
    int segundos;
    int milissegundos;
}Intervalo;

Intervalo* VerificaIntervalo(struct timeval* inicio, struct timeval* final) {
    Intervalo* intervalo = (Intervalo*)malloc(sizeof(Intervalo));

    if (inicio->tv_sec == final->tv_sec) {
        intervalo->segundos = 0;
        intervalo->milissegundos = final->tv_usec - inicio->tv_usec;
    }
    else {
        intervalo->milissegundos = 1000000 - inicio->tv_usec;
        intervalo->segundos = final->tv_sec - (inicio->tv_sec + 1);
        intervalo->milissegundos += final->tv_usec;
        if (intervalo->milissegundos >= 1000000) {
            intervalo->milissegundos -= 1000000;
            intervalo->segundos += 1;
        }
    }
    return intervalo;
}
...
void ProcessoPrimario(int qtdProc) {
    ...
    for (inicio = 1; inicio < qtdProc; inicio++) {
        for (i = div; i < (inicio + 1) * processoAtu; i++) {
            MPI_Recv(bufferRecebimento, ORDEM, MPI_INT, inicio, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            for (j = 0; j < ORDEM; j++) {
    ...
    for (atual = 0; atual < QTD_GEN; atual++) {
        ...
        for (inicio = 1; inicio < qtdProc; inicio++) {
            for (i = (processoAtu * inicio); i < (processoAtu * (inicio + 1)); i++) {
                MPI_Recv(bufferRecebimento, ORDEM, MPI_INT, inicio, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                for (j = 0; j < ORDEM; j++) {
                    matriz[i][j] = bufferRecebimento[j];
                }
    ...
    tempoDecorrido = VerificaIntervalo(&inicio, &final);
    printf("Tempo: %d,%d s\n", tempoDecorrido->segundos, tempoDecorrido->milissegundos);
}
